# Triggering reload to update roles - 2026-02-15
...
def seed_demo_results():
    """Seed some initial results for demo purposes if none exist."""
    if QuizResult.query.first() or UserInteraction.query.first():
        return
    
    users = User.query.all()
    simulations = Simulation.query.all()
    if not users or not simulations:
        return
        
    now = datetime.now(timezone.utc)
    for user in users:
        # 1-3 quizzes for each user
        for _ in range(random.randint(1, 3)):
            score = random.randint(50, 100)
            res = QuizResult(user_id=user.id, score=score, total_questions=10, 
        ...
        for _ in range(random.randint(2, 5)):
            ...
            action = sim.correct_action if is_correct else 'unsafe'
            inter = UserInteraction(user_id=user.id, simulation_id=sim.id, 
                                    action_taken=action, is_correct=is_correct,
                                    timestamp=now - timedelta(days=random.randint(0, 7)))
...
def login():
    ...
    return jsonify({
        ...
        'user': {
            ...
            'email': user.email, 
            'score': user.score,
            'role': getattr(user, 'role', 'System User/Employee')  # Default to 'System User/Employee' if column missing
        }
    })
    ...
    try:
        ...
        for u in users:
            out.append({
                'id': u.id,
                'name': u.name,
                'role': getattr(u, 'role', 'System User/Employee'),
                'email': u.email,
                'score': u.score or 0,
                'created_at': None
            })
...
def seed_database():
    with app.app_context():
        ...
        # Seed demo results for Cyber Health & Leaderboard
        try:
            seed_demo_results()
        except Exception as e:
            print(f"Demo results seeding failed: {e}")
        ...
        try:
            ...
            if bp.name in app.blueprints:
                ...
                if modname == 'routes.cyber_health':
                    print("[OK] CYBER HEALTH DASHBOARD READY")
        except Exception as e:
